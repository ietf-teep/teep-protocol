...
: The value in the token parameter is used to match responses to requests.
  ...
  (64 bits) and maximum of 64 bytes, which is the same as Nonce Claim in EAT.
  (see {{I-D.ietf-rats-eat}} Section 3.3). The first usage of a token
  generated by TAM MUST be randomly created so the device will unlikely
  receive the same token value when the device handles multiple TAMs.
  Consequent token value MUST be a different value for each request message
